...
In Section \ref{sec:intro:problem-statement} I extensively elaborate on the reasons why I opted to focus on these four types of AS.
...
In this dissertation, we tackle the problem that practitioners face when including architectural smells as part of their TD management activities.
To better explain the problem, let us quickly look at the key phases of TD management \cite{Li2015} when the type of TD managed are architectural smells: (1) \emph{identify} what elements in the system are affected by AS and the type of the smells; (2) \emph{quantify} the impact of each smell on the development activities (both maintenance and evolution); (3) \emph{prioritise} the smells in order to determine the urgency to refactor %-- i.e. decide on whether to keep the smell (i.e. let it accrue interest) or refactor it (i.e. pay back the debt); 
(4) \emph{repay} the debt by refactoring smells according to the prioritisation strategy adopted; and finally (5) regularly \emph{monitor} the evolution of AS over time.

The TD management problem stemming from the presence of architectural smells is that the current research landscape contains plenty of studies that focus on the first phase of TD management for architectural smells -- i.e.  \emph{identification} -- whereas the work done on the other phases is not as mature.
...
However, the scarcity of research in the phases from quantification through monitoring never allowed proper AS management to materialise in the real world, leaving practitioners to rely on intuition, assumptions and experience, instead of \emph{data} and \emph{well-established practices}, in order to make refactoring decisions.
In the long run, this is likely to result in a less sustainable software \cite{Winters2020} and an increased amount of technical debt accrued. 
%Ultimately, the scarcity of research in the phases from quantification through monitoring severely limits the options available for practitioners, who have to rely on intuition, instead of data, and on experience, instead of well-established practices, in order to make refactoring decisions.
