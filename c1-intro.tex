...
The smells detected need to be quantified in terms of TD, prioritised, monitored and repayed in order for AS management to be effective.
This hinders practitioners' ability to manage architectural TD, ultimately affecting the sustainability of the system.

As a first step towards addressing this issue, I decided to investigate how AS evolve in real world systems and formulated RQ1: \textit{``How do AS evolve in open source systems?''} 
...
Answering RQ1 required analysing 524 releases belonging to 15 open source pojects.
...
The goal of RQ2 is two-fold: (1) understand the issues stemming from the presence of AS and that affect maintainability; and (2) study the opinion of practitioners on AS in terms of introduction and how AS are dealt with.
...
This RQ is similar to RQ1, but focuses specifically on the co-occurrence and introduction order of smells and takes into account also the experiences of practitioners.
...
A common theme arised from RQ3 is that most of the practitioners we talked to were concerned of the frequency of change of components and its propagation to other components due to the presence of smells. 
...
This helps understanding the relation that AS have with the amount of interest paid by practitioners that decide to not repay the debt represented by smells.
Answering this RQ required investigating 27 projects, 360 years of total development and analyse over 305 millions of lines of code.

Research questions from 1 through 4 all focus on studying the problem; indeed, they are \emph{knowledge questions} and their answers tell us exactly how smells evolve, how they are perceived by practitioners, how they relate with changes in the source code of the system, and ultimately allow us to understand how to prioritise, quantify, and monitor smells.
In contrast, RQ5 is a \emph{design problem} to \textit{``Design an approach to estimate the technical debt principal generated by AS''}
...
This means that TD management decisions are affected by other qualities as well, and to better understand to extent of these trade-offs I formulated RQ6: \textit{``How are trade-offs between quality attributes currently managed by the industry?''}
...
\begin{table}[]
    \footnotesize
    \centering
    \caption{Overview of the research methodologies adopted in this dissertation.}
    \label{c1:tab:overview-methodology}
    \begin{tabular}{@{}lm{5cm}ll@{}}
    \toprule
    \textbf{Code} & \textbf{Research Question} & \textbf{Empirical Method} & \textbf{Described in} \\ \midrule
    RQ1 & \textit{How do AS evolve in open source systems?} & Case study & Section \ref{c2:sec:case-study} \\
    RQ2 & \textit{How are AS perceived by industrial practitioners?} & Case study & Section \ref{c3:sec:case-study}  \\
    RQ3 & \textit{How do AS evolve in industrial embedded systems?} & Case study & Section \ref{c4:sec:case-study} \\
    RQ4 & \textit{How do AS correlate to changes in the source code of the system?} & Case study & Section \ref{c5:sec:design} \\
    RQ5 & \textit{Design an approach to estimate the technical debt principal generated by AS} & Case study & Section \ref{c6:sec:study-design} \\
    RQ6 & \textit{How are trade-offs between quality attributes currently managed by the industry?} & Case study & Section \ref{c7:sec:case-study} \\ \bottomrule
    \end{tabular}
\end{table}
