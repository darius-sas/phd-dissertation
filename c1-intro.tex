...
To further scope down the problem, we considered the fact that dozens of architectural smells have been reported in books and scientific literature. Thus, we decided to focus our attention only on the four architectural smells (CD, HL, GC, UD -- see Section \ref{c2:sec:arch-smells}) that are the most prominent in the literature\cite{Azadi2019}; two of them are also well known in the industry \cite{Lippert2006,Martin2018}.
Moreover, the literature also provides manually-validated, open source tools that we can use to detect the smells.
...
Studying the evolution of AS entails studying two aspects: the evolution of the individual AS instances and the persistence of AS instances within the system. This study required analysing 524 releases belonging to 15 open source projects.
...
The investigation of RQ1, being a purely quantitative analysis based on the mining of software repositories, cannot provide any further insight on why practitioners introduce smells or whether they consider the consequences on software maintenance and evolution.
...
Finally, we set up a case study to validate our approach by interviewing 16 practitioners (from both the open source and industrial worlds) and ensure that the output provided is a relevant and meaningful estimate of the TD principal incurred by a smell.

During the validation of the solution in RQ5, we discovered an interesting phenomenon: while practitioners understand and relate to the amount of TD principal, they often have to tolerate TD, hence sacrificing maintainability, in favor of other qualities.
...
Table \ref{c1:tab:overview-methodology} lists the empirical methods that were used to answer each knowledge question, as well as the corresponding sections in this thesis elaborating on the study design.
...
The empirical methods adopted in this PhD project are the following:
\begin{description}
    ...
    It is important to mention that grounded theory also describes how data can be collected; however, in this thesis, only the qualitative data analysis part of grounded theory was used (i.e. the constant comparative method\cite{Glaser2017}).
...
\begin{table}[]
    ...
    RQ5 & \textit{Design an approach to estimate the technical debt principal generated by AS} & Chapter \ref{chap:6} & TSE* \\
